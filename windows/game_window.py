...
class GameWindow(QMainWindow):
    ...
    def make_a_shift_in_the_field(self, direction: str) -> None:
        if direction == 'right' \
            ...
            self.update_labels_with_intervals()
                ...
                and self.interval_y[1] < self.one_field_size[1] - 1:
            self.hide_all_field_button()
            self.interval_y = self.interval_y[0] + 16, self.interval_y[1] + 16
            self.show_area_field_button(
                self.current_level, area=(self.interval_x, self.interval_y))
    ...
    def update_info_on_label(self, data_first_lvl: dict,
        ...
        for key in data_first_lvl.keys():
            ...
            self.label_fields_first_level[
                translator_first_lvl[key]].setText(first_text)
            if self.three_dimensional:
                second_text = str(data_second_lvl[key])
                self.label_fields_second_level[
                    translator_second_lvl[key]].setText(second_text)
        ...
        if self.three_dimensional:
            label_fields_second_level = \
                self.create_changed_labels(
                    data_for_fields['submarine'], hide=True)
        return label_fields_first_level, label_fields_second_level

    def create_changed_labels(self, data: Dict[str, List[int]],
    ...
    def create_button_to_change_levels(self) -> QtWidgets.QPushButton:
        ...
        button_to_change_levels.clicked.connect(
            partial(self.change_levels, button_to_change_levels))
        return button_to_change_levels

    def change_levels(self, button_to_change_levels: QtWidgets.QPushButton) ->\
            None:
        reverse_phrases = {
        ...
        self.change_the_display_unchanged_labels(
            self.labels_second_lvl, show=result)
        self.change_the_display_unchanged_labels(
            self.labels_first_lvl, show=not result)

        self.change_the_display_changed_labels(
            self.label_fields_second_level, show=result)
        self.change_the_display_changed_labels(
            self.label_fields_first_level, show=not result)
    ...
    @staticmethod
    def change_the_display_changed_labels(data: Dict[str, QtWidgets.QLabel],
                                          show: bool) -> None:
    ...
    @staticmethod
    def change_the_display_unchanged_labels(data: List[QtWidgets.QLabel],
                                            show: bool) -> None:
        for label in data:
            if show:
                label.show()
    ...
    def update_labels_with_intervals(self):
        self.labels_for_display_interval['x_start_player']\
            .setText(str(self.interval_x[0]))
        self.labels_for_display_interval['x_end_player']\
            .setText(str(self.interval_x[1]))
        self.labels_for_display_interval['y_start_player']\
            .setText(str(self.interval_y[0]))
        self.labels_for_display_interval['y_end_player']\
            .setText(str(self.interval_y[1]))

        self.labels_for_display_interval['x_start_enemy']\
            .setText(str(self.interval_x[0]))
        self.labels_for_display_interval['x_end_enemy']\
            .setText(str(self.interval_x[1]))
        self.labels_for_display_interval['y_start_enemy']\
            .setText(str(self.interval_y[0]))
        self.labels_for_display_interval['y_end_enemy']\
            .setText(str(self.interval_y[1]))
    ...
    def create_field_buttons(self, start_x: int, is_a_player: bool) -> {}:
        ...
        x_coordinate_grid, y_coordinate_grid \
            = self.get_coordinate_grid(start_x)
